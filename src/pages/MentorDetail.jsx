...
export default function MentorDetail() {
  ...
  async function checkAuthLinked() {
    const { data } = await supabase.from('profiles').select('id').eq('mentor_id', id).limit(1)
  ...
  async function handleSendInvite() {
    ...
    const { data, error: fnError } = await supabase.functions.invoke('invite-user', {
      body: {
        email: form.email, role: 'mentor', entity_id: id,
    ...
    if (errText) {
      ...
      const msg = data?.added_role
        ? `Mentor role added to existing account (${form.email}). They can now switch to the Mentor dashboard.`
        : `Account created. A password reset email has been sent to ${form.email}.`
      setEmailActionMsg({ type: 'success', text: msg })
      if (data?.temp_password) { setTempPassword(data.temp_password); setShowPassword(false); setPwCopied(false) }
      setAuthLinked(true)
    }
  }
  ...
  async function handleSave(e) {
    ...
    const { error: updateError } = await supabase.from('mentors').update({
      first_name: form.first_name, last_name: form.last_name,
    ...
    if (form.linked_assistant_mentor_id) {
      await supabase.from('assistant_mentors').update({
        first_name: form.first_name, last_name: form.last_name,
        email: form.email, phone: form.phone,
        address_street1: form.address_street1, address_street2: form.address_street2,
        address_city: form.address_city, address_state: form.address_state,
        address_zip: form.address_zip, address_country: form.address_country,
        messaging_methods: form.messaging_methods, preferred_messaging: form.preferred_messaging,
      }).eq('id', form.linked_assistant_mentor_id)
    }

    if (avatarFile) {
      const result = await uploadAvatar(avatarFile, 'mentors', id)
      if (result.error) { setError(`Saved but photo upload failed: ${result.error}`); setSaving(false); return }
      const { error: urlError } = await supabase.from('mentors').update({ avatar_url: result.publicUrl }).eq('id', id)
    ...
    <div style={st.container}>
      <div style={st.header}>
        ...
        <div>
          ...
          <p style={st.subtitle}>Mentor since {form.start_date ? new Date(form.start_date).toLocaleDateString() : '—'}</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSave}>
        <div style={st.twoCol}>
          ...
          <div style={st.col}>
            <Section title="Compensation">
              <div style={st.fieldGroup}>
                ...
                <span style={st.payTypeHint}>
                  {(form.pay_type === 'percentage' || !form.pay_type) && 'Mentor earns a percentage of each mentee\'s arrangement subscription.'}
                  {form.pay_type === 'monthly' && 'Mentor receives a fixed dollar amount each month.'}
                  {form.pay_type === 'per_meeting' && 'Mentor is paid a flat rate for every completed meeting.'}
                  {form.pay_type === 'hourly' && 'Mentor is paid per hour worked. Hours tracking is in development.'}
                </span>
              </div>
            ...
            <Section title="Notes">
              <textarea name="notes" value={form.notes || ''} onChange={handleChange} rows={3} placeholder="Any notes about this mentor..." style={{ ...st.input, resize: 'vertical' }} />
            </Section>
              ...
              {tempPassword && (
                <div style={st.tempPwCard}>
                  ...
                  <div style={st.tempPwRow}>
                    <code style={st.tempPwCode}>{showPassword ? tempPassword : '••••••••••••••••'}</code>
                    <button type="button" style={st.tempPwBtn} onClick={() => setShowPassword(v => !v)}>{showPassword ? <EyeOff size={15} /> : <Eye size={15} />}</button>
        ...
        <div style={st.formActions}>
          <button type="button" style={st.cancelBtn} onClick={() => navigate('/admin/mentors')}>Cancel</button>
...
const st = {
  ...
  saveBtn: { padding: '0.55rem 1.4rem', background: 'var(--primary-gradient)', color: '#fff', border: 'none', borderRadius: 7, cursor: 'pointer', fontWeight: 600, fontSize: '0.875rem' },
