...
export default function StaffDetail() {
  ...
  async function fetchStaff() {
    const { data, error: err } = await supabase.from('staff').select('*').eq('id', id).single()
    if (err) { setError(err.message); return }
    setForm(data)
  }

  async function checkAuthLinked() {
    const { data } = await supabase.from('profiles').select('id').eq('staff_id', id).limit(1)
  ...
  async function handleSendInvite() {
    ...
    const { data, error: fnError } = await supabase.functions.invoke('invite-user', {
      body: {
        email: form.email, role: 'staff', entity_id: id,
    ...
    if (errText) {
      ...
      const msg = data?.added_role
        ? `Staff role added to existing account (${form.email}). They can now switch dashboards.`
  ...
  async function handleSave(e) {
    ...
    const { error: updateError } = await supabase.from('staff').update({
      ...
      pay_rate: form.pay_rate !== '' && form.pay_rate != null ? parseFloat(form.pay_rate) : null,
    }).eq('id', id)

    if (updateError) { setError(updateError.message); setSaving(false); return }

    if (avatarFile) {
      const result = await uploadAvatar(avatarFile, 'staff', id)
      if (result.error) { setError(`Saved but photo upload failed: ${result.error}`); setSaving(false); return }
      const { error: urlError } = await supabase.from('staff').update({ avatar_url: result.publicUrl }).eq('id', id)
  ...
  return (
    <div style={st.container}>
      <div style={st.header}>
        <button style={st.back} onClick={() => navigate('/admin/staff')}>← Staff</button>
        <AvatarUpload
          url={form.avatar_url}
        ...
        <div>
          ...
          <p style={st.subtitle}>
            {form.role_title || 'Staff'}{form.start_date ? ` · Since ${new Date(form.start_date).toLocaleDateString()}` : ''}
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSave}>
          ...
          <div style={st.col}>
            <Section title="Compensation">
              <div style={st.fieldGroup}>
                ...
                <select name="pay_type" value={form.pay_type || ''} onChange={handleChange} style={st.input}>
              ...
              {form.pay_type && (
                <div style={st.fieldGroup}>
                  <label style={st.label}>{form.pay_type === 'hourly' ? 'Hourly Rate' : 'Annual Salary'}</label>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                    <span style={{ fontSize: '1rem', color: '#4a5568', fontWeight: 600 }}>$</span>
                    <input
                      style={{ ...st.input, width: '140px' }}
                      type="number" name="pay_rate"
                      value={form.pay_rate ?? ''}
                      onChange={handleChange}
                      min="0" step="0.01" placeholder="0.00"
                    />
                    <span style={{ fontSize: '0.85rem', color: '#718096' }}>{form.pay_type === 'hourly' ? '/hr' : '/yr'}</span>
                  </div>
                </div>
              )}
            </Section>
        ...
        <div style={st.formActions}>
          <button type="button" style={st.cancelBtn} onClick={() => navigate('/admin/staff')}>Cancel</button>
...
const st = {
  ...
  back: { background: 'none', border: 'none', color: '#f59e0b', cursor: 'pointer', fontSize: '0.9rem', fontWeight: 600, padding: 0 },
  ...
  saveBtn: { padding: '0.55rem 1.4rem', background: 'linear-gradient(135deg, #f59e0b, #f97316)', color: '#fff', border: 'none', borderRadius: 7, cursor: 'pointer', fontWeight: 600, fontSize: '0.875rem' },
