...
import type {
  Offering, Lesson, LessonQuestion, MenteeOffering,
...
export default function MenteeCourseViewerPage() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      setLoading(true)
      try {
        // Fetch the mentee_offering with its offering
        ...
        const enriched: LessonWithProgress[] = allLessons.map(l => ({
          ...l,
  ...
  // Fetch questions + responses when lesson changes
  useEffect(() => {
    if (!selectedLessonId || !menteeId || !id) { setQuestions([]); setResponses({}); return }
    ...
    async function fetchQuestions() {
      ...
      try {
        ...
        if (rRes.data) {
          for (const r of rRes.data as QuestionResponse[]) {
    ...
    async function markStarted() {
      ...
      if (selectedLesson!.progress) {
        ...
        await supabase
          ...
          .eq('id', selectedLesson!.progress!.id)
      } else {
        // Insert new
        ...
        if (data) {
          setLessons(prev => prev.map(l =>
            l.id === selectedLesson!.id ? { ...l, progress: data as LessonProgress } : l
          ))
        }
      }
    }
  ...
  const saveResponse = useCallback(async (questionId: string, responseText: string | null, selectedOptionIndex: number | null, isCorrect: boolean | null) => {
    ...
    if (existing) {
      ...
      if (!e) {
        setResponses(prev => ({
          ...prev,
          [questionId]: { ...existing, response_text: responseText, selected_option_index: selectedOptionIndex, is_correct: isCorrect, answered_at: now },
        }))
      }
    } else {
      ...
      if (data && !e) {
        setResponses(prev => ({ ...prev, [questionId]: data as QuestionResponse }))
      }
    }
    ...
    try {
      ...
      setLessons(prev => prev.map(l =>
        l.id === selectedLesson.id
          ? { ...l, progress: { ...(l.progress ?? {} as LessonProgress), status: 'completed', completed_at: now, updated_at: now } as LessonProgress }
          : l
      ))

      // Check if all lessons completed -> mark course as completed
      const updatedLessons = lessons.map(l =>
        l.id === selectedLesson.id ? { ...l, progress: { status: 'completed' } } : l
      )
      const allComplete = updatedLessons.every(l => l.progress?.status === 'completed')
      if (allComplete) {
        await supabase
          .from('mentee_offerings')
        ...
        if (nextLesson) {
          setTimeout(() => setSelectedLessonId(nextLesson.id), 800)
        }
      }
    } catch (err) {
      setMsg({ type: 'error', text: (err as Error).message || 'Failed to complete lesson' })
    } finally {
      setSaving(false)
    }
  }
  ...
  if (error || !course || !menteeOffering) {
    return (
      <div className="max-w-3xl">
        <button onClick={() => navigate('/my-courses')} className="text-sm text-gray-500 hover:text-gray-700 transition-colors mb-4">&larr; Back to My Courses</button>
    ...
    <div className="max-w-6xl">
      ...
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-4">
          <button onClick={() => navigate('/my-courses')} className="text-sm text-gray-500 hover:text-gray-700 transition-colors">&larr; Back</button>
          <div>
      ...
      {/* Status message */}
      {msg && (
        <div className={`mb-4 flex items-start gap-3 rounded border px-3 py-2 text-sm ${
            ...
            <div className="divide-y divide-gray-50 overflow-y-auto" style={{ maxHeight: 'calc(100vh - 260px)' }}>
              {lessons.map((lesson, index) => {
                ...
                return (
                  <button
                    key={lesson.id}
                    onClick={() => { setSelectedLessonId(lesson.id); setMsg(null) }}
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      <p className={`text-xs font-medium truncate ${isSelected ? 'text-gray-900' : 'text-gray-700'}`}>
                        {lesson.title}
                      </p>
          ...
          {!selectedLesson ? (
            <div className="bg-white rounded-md border border-gray-200/80 px-6 py-16 text-center">
              <p className="text-sm text-gray-400">
                {lessons.length === 0 ? 'No lessons in this course yet.' : 'Select a lesson to begin.'}
              </p>
            </div>
          ) : (
            <div className="space-y-4">
              ...
              <div className="bg-white rounded-md border border-gray-200/80 px-5 py-4">
                <div className="flex items-center justify-between">
                  ...
                  {isLessonCompleted && (
                    <span className="text-[10px] font-medium px-2 py-1 rounded bg-green-100 text-green-600 shrink-0">
                      Completed
                    </span>
                  )}
                </div>
                  ...
                  <div className="space-y-5">
                    {questions.map((q, qi) => (
                      <MenteeQuestionCard
                        ...
                        onSave={saveResponse}
                        disabled={isLessonCompleted ?? false}
                      />
                    ))}
                  </div>
                </div>
              ...
              {!isLessonCompleted && (
                <div className="flex items-center gap-3">
                  <button
                    ...
                    disabled={saving}
                    className="rounded bg-brand px-5 py-2.5 text-sm font-medium text-white hover:bg-brand-hover focus:outline-none focus:ring-2 focus:ring-brand focus:ring-offset-2 disabled:opacity-60 disabled:cursor-not-allowed transition"
                  >
                    {saving ? 'Saving...' : 'Complete Lesson'}
                  </button>
                  {questions.length > 0 && Object.keys(responses).length < questions.length && (
                    <p className="text-xs text-amber-600">
                      {questions.length - Object.keys(responses).length} question{questions.length - Object.keys(responses).length !== 1 ? 's' : ''} unanswered
                    </p>
                  )}
                </div>
              )}
                ...
                {(() => {
                  ...
                  return (
                    <>
                      {prev ? (
                        <button
                          onClick={() => { setSelectedLessonId(prev.id); setMsg(null) }}
                      ...
                      {next ? (
                        <button
                          onClick={() => { setSelectedLessonId(next.id); setMsg(null) }}
                          className="flex items-center gap-1.5 text-xs font-medium text-gray-500 hover:text-gray-700 transition-colors"
                        >
                          {next.title}
                          <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
...
// ── Mentee Question Card ──

function MenteeQuestionCard({
  question,
  index,
  response,
  onSave,
  ...
  // Sync when response prop changes
  useEffect(() => {
    if (response) {
      setText(response.response_text ?? '')
      setSelectedOption(response.selected_option_index ?? null)
      setSubmitted(true)
      if (isQuiz) setShowResult(true)
    }
  }, [response?.id])
  ...
  async function handleSubmit() {
    if (isQuiz) {
      ...
      if (!text.trim()) return
      await onSave(question.id, text.trim(), null, null)
    }
    setSubmitted(true)
  }
  ...
  return (
    <div className="rounded-lg border border-gray-200 p-4">
      <div className="flex items-center gap-2 mb-3">
        ...
        {submitted && (
          <span className="text-[10px] font-medium px-1.5 py-0.5 rounded bg-green-50 text-green-600">Answered</span>
        )}
      </div>
        ...
        <div className="space-y-2">
          ...
          {!submitted && !disabled && (
            <button
              ...
              className="mt-2 px-3 py-1.5 text-xs font-medium text-white bg-brand rounded hover:bg-brand-hover disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
              Submit Answer
            </button>
          )}
